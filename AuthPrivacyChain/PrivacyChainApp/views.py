...
def readDetails(contract_type):
    ...
    deployed_contract_address = '0x3A8bC0046f1C48DF60ED62b1961Dc900787b346B' #hash address to access AuthPrivacyChain contract
...
def saveDataBlockChain(currentData, contract_type):
    ...
    deployed_contract_address = '0x3A8bC0046f1C48DF60ED62b1961Dc900787b346B' #contract address
    with open(compiled_contract_path) as file:
        contract_json = json.load(file)  # load contract info as JSON
        contract_abi = contract_json['abi']  # fetch contract's abi - necessary to call its functions
    file.close()
    ...
    if contract_type == 'signup':
        details+=currentData
        msg = contract.functions.createDataUser(details).transact({"from":web3.eth.defaultAccount})
        tx_receipt = web3.eth.get_transaction_receipt(msg)
    if contract_type == 'direct':
        details+=currentData
        msg = contract.functions.setDirectSharing(details).transact({"from":web3.eth.defaultAccount})
        tx_receipt = web3.eth.get_transaction_receipt(msg)
    if contract_type == 'indirect':
        details+=currentData
        msg = contract.functions.setInDirectSharing(details).transact({"from":web3.eth.defaultAccount})
        tx_receipt = web3.eth.get_transaction_receipt(msg)
...
def Download(request):
    if request.method == 'GET':
        filename = request.GET['t1']  # Get filename from request
        hashcode = request.GET['t2']  # Get IPFS hash from request

        # Retrieve the encrypted file from IPFS
        encrypted_content = api.cat(hashcode)

        # Decrypt the file
        decrypted_content = decrypt(pickle.loads(encrypted_content))
...
def IndirectAccess(request):
    if request.method == 'GET':
        global username, access_user
        username = request.session['username']
        access_user = request.session['access_user']
        readDetails('direct')
        arr = details.split("\n")
...
def DataOwnerLogin(request):
    if request.method == 'GET':
       return render(request, 'DataOwnerLogin.html', {})
    ...
    if request.method == 'POST':
        ...
        for i in range(len(arr)-1):
            ...
            if array[1] == username and password == array[2] and array[3] == usertype:
                ...
                request.session['access_user'] = access_user
                break
        if status != 'none' and usertype == 'Data Owner':
            ...
            if usertype == "Data Owner":
                return render(request, 'DataOwnerLogin.html', context)
            else:
